...
export const JackpotWinnerCard: React.FC<JackpotWinnerCardProps> = ({ id, ticketCode }) => {
  ...
  return (
    ...
    >
       <div className="bg-bg-base rounded-3xl sm:rounded-[2rem] border border-accent/40 shadow-xl shadow-accent/40 transition-all duration-500 hover:shadow-2xl hover:shadow-accent/60 hover:border-accent/80 group-hover:-translate-y-1 p-5 sm:p-8 relative flex flex-col sm:flex-row sm:items-center justify-between gap-6 sm:gap-8 min-h-[120px] overflow-hidden">
             {/* Glows */}
             ...
             <div className="flex items-center gap-5 sm:gap-8 relative z-10">
                <div className="w-14 h-14 sm:w-24 sm:h-24 bg-gradient-to-br from-accent/20 to-accent/5 text-accent rounded-xl sm:rounded-2xl flex items-center justify-center relative border border-accent/20 shadow-inner group-hover:scale-105 transition-transform duration-500">
                   ...
                   <div className="absolute inset-0 bg-accent rounded-xl sm:rounded-2xl blur-xl opacity-[0.15] animate-pulse"></div>
                </div>
             ...
             <Button 
               ...
               important
               className="w-full sm:w-auto px-6 sm:px-12 h-12 sm:h-16 uppercase tracking-[0.2em] relative z-10 font-black sm:text-lg rounded-xl sm:rounded-2xl shadow-[0_0_20px_rgba(0,217,145,0.4)] hover:shadow-[0_0_30px_rgba(0,217,145,0.6)]"
             >
               NHẬN THƯỞNG
             </Button>
       </div>
